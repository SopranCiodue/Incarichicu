...
export class IncarichiListOldComponent implements OnInit, AfterViewInit {
  ...
  setupFilterAndSubscription(
    ...
    dataSource: MatTableDataSource<IIncarichi>,
    listAllegati: IAllegatiList[],
  ) {
    dataSource.filterPredicate = (data: IIncarichi, filter: string) => {
      ...
      const matchingRows = listAllegati.filter(
        (allegato) => allegato.keyord === data.key_ord,
      );
  ...
  toggleExpandedElement(row: IIncarichi) {
    ...
    this.incarichiService.setSelectedIncarichiData(
      row.key_ord,
      row.haccp,
      row.prendiAllegato,
      row.tipologia,
  ...
  aggiornamentoFiltro() {
    ...
    this.incarichiSubcription.add(
      this.setupFilterAndSubscription(
        this.incarichiService,
        this.dataSource,
        this.listAllegati,
      ),
